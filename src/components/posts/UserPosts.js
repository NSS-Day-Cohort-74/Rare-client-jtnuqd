...
export const UserPosts = ({ token }) => {
  ...
  useEffect(() => {
    ...
    getPostCategoryAndUser(token).then((data) => setPosts(data));
    console.log({ "all my posts": posts });
  }, []);

  useEffect(() => {
    setMyPosts(posts.filter((p) => p.user_id == token));
  }, [posts, token]);
                    ...
                    <div className="content">
                      <div className="level mb-2">
                        <div className="level-left">
                          <Link to={`/posts/${post.id}`} className="title is-4">
                            {post.title}
                          </Link>
                        </div>
